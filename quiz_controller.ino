//////Pins Definitions//////
const int pin_led_0 = 7;                      // pin value for led0 (far right)
const int pin_led_1 = 6;                      // pin value for led1
const int pin_led_2 = 5;                      // pin value for led2
const int pin_led_3 = 4;                      // pin value for led3 (far left)
...
const int pin_led_submit = 13;                // pin connected to blue LED indicating when buttons have been held long enough to subbmit answer
...
const int pin_answerkey_0 = 8;                // pin value of correct button state for A from answer_key
const int pin_answerkey_1 = 9;                // pin value of correct button state for B from answer_key
const int pin_answerkey_2 = 10;               // pin value of correct button state for C from answer_key
const int pin_answerkey_3 = 11;               // pin value of correct button state for D from answer_key
...
int time_submit = 2;                          // How long buttons need to be held before answer can be submitted (allows for multiselect)
...
long score_total = 0.00;                      // Stores value of total score 
...
volatile int secondsCounterAnswer = 0;              // Tracks how long buttons have been held
...
volatile bool getAnswerFlag = false;                // Flag to indicate when to save state of answer ()
volatile bool right_answer = false;                 // Flag to indicate if answer is right or wrong
volatile bool question_asked = false;               // Flag to indicate when question needs to be displayed
volatile bool end_stats_printed = false;            // Flag to pint end of quiz stats only once
...
void setup() {
  ...
  pinMode(pin_correct, INPUT);                      // Initialize correct/incorrect answer pin as INPUT
  ...
  interrupts();                                     // Re-enable global interrupts  //set "interrupt()" function to run when any button's state has been changed
// Attach external interrupt to trigger on both button push and release
...
void loop(){                                       // Run Code
  if(current_question <= 10){                      // For questions 1-10:
    ...
    get_answer();                                  // Run function to update score, bonus points, display answer
  ...
  if(current_question >10){                        // End quiz once complete
    end_of_quiz_stats();                           // Print quiz stats
    while(end_reset == false){  }                  // Stall until reset button is pressed
  }
}
/////////////////////////////////////////////////////
/////////////////////////////////////////////////////
